...
// URL: https://www.codechef.com/FEB21B/problems/SUMXOR2
// Memory Limit: 256 MB
// Time Limit: 2000 ms
//
...
void fourier(int *a, int n, int f = 0) {
  ...
  for (m = 2; m <= n; m <<= 1)
    ...
    for (i = 0, wm = binpow(3, f == 1 ? (M - 1) / m : (M - 1) / m * (m - 1), M);
         i < n; i += m)
  ...
  if (f == 1)
    for (w = binpow(n, M - 2, M), i = 0; i < n; i++) a[i] = 1ll * a[i] * w % M;
}
...
int nCr(int n, int r) {
  ...
  return (((inv_fact[r] * inv_fact[n - r]) % mod) * factorial[n]) % mod;
}

void prepare() {
  inv_fact[1] = inv_fact[0] = 1;

  book_keep[1] = book_keep[0] = 1;

  factorial[1] = factorial[0] = 1;

  for (int i = 2; i < 200005; i++) {
    book_keep[i] = book_keep[mod % i] * (mod - mod / i) % mod;
  }
  ...
  for (int i = 2; i < 200005; i++) {
    inv_fact[i] = (book_keep[i] * inv_fact[i - 1]) % mod;
  }
}
...
signed main() {
  ...
  for (int ii = 0; ii < 30; ii++) {
    ...
    for (int i = 1; i <= book_keep[ii]; i += 2) {
      eno.push_back(nCr(book_keep[ii], i));
    ...
    while (i <= (n - book_keep[ii])) {
      orez.push_back(nCr(n - book_keep[ii], i));
      ;
