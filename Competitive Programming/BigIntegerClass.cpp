/*
<<<<<<<<<<<---APPROACH-->>>>>>>>>>>>>>
This is a Big Integer Class, implemented in C++,
for handling very large numbers, or numbers greater than unsigned long long int in a 64 bit
implementation. So, it can easily operate on numbers greater than 10^18.


Installation
Just include the file as a preprocessor directive, like : #include "name"
where name denotes your filename, the name you saved for this Big Int file. Also,
ensure that you keep it in the same directory as your working file, else you have to
write the complete path in that place.

Features
Addition (shorthand notation too)
Subtraction (shorthand notation too)
